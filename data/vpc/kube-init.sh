...
ssh_options="-i /home/kubework/keys/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"
...
control_plane_execute "kubeadm init --apiserver-advertise-address=$control_plane_lan_ip --apiserver-cert-extra-sans=$control_plane_wan_ip --pod-network-cidr=10.244.0.0/16"
control_plane_execute "mkdir -p /root/.kube"
control_plane_execute "cp -f /etc/kubernetes/admin.conf /root/.kube/config"
control_plane_execute "chown $(id -u):$(id -g) /root/.kube/config"
control_plane_execute "export KUBECONFIG=/etc/kubernetes/admin.conf"
control_plane_execute "kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml"
